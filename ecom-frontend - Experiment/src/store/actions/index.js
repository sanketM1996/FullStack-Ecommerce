  ...
  (sendData, toast, reset, navigate, setLoader) => async (dispatch, getState) => {
    try {
      ...
      if (guestCart.length > 0) {
        // ✅ If guest cart exists, sync it to server
        await dispatch(createUserCart({ products: guestCart }));

        // Clear guest cart from localStorage (server will now manage it)
        localStorage.removeItem("cartItems");

        // Fetch updated cart from server
        await dispatch(getUserCart());
      } else {
        // ✅ If no guest cart, just load user’s existing server cart
        await dispatch(getUserCart());
      }
...
export const updateOrderStatusFromDashboard =
  (orderId, orderStatus, toast, setLoader, isAdmin, currentPage = 1, pageSize = 10) =>
  async (dispatch) => {
    try {
      setLoader(true);
      const endpoint = isAdmin ? "/admin/orders/" : "/seller/orders/";
      ...
      const queryString = `page=${currentPage - 1}&size=${pageSize}`;
...
export const updateProductFromDashboard = 
  (sendData, toast, reset, setLoader, setOpen, isAdmin, currentPage, pageSize) => async (dispatch) => {
    try {
        setLoader(true);
        const endpoint = isAdmin ? "/admin/products/" : "/seller/products/";
        ...
        reset();
        setOpen(false);
        await dispatch(dashboardProductsAction(`page=${currentPage}&size=${pageSize}`, isAdmin)); // ✅ refresh page
    } catch (error) {
...
//   user
// ✅ Fetch all users (paginated + sorting)
export const fetchUsers =
  (pageNumber = 0, pageSize = 10, sortBy = "userId", sortDir = "asc") =>
  async (dispatch) => {
    try {
      dispatch({ type: "IS_FETCHING" });
      const { data } = await api.get(
        `/admin/user?pageNumber=${pageNumber}&pageSize=${pageSize}&sortBy=${sortBy}&sortDir=${sortDir}`
      );
...
// ✅ Create user
export const createUser = (userData, toast, resetForm) => async (dispatch) => {
  try {
...
// ✅ Update user
export const updateUser = (id, userData, toast) => async (dispatch) => {
  try {
...
// ✅ Delete user
export const deleteUser = (id, toast) => async (dispatch) => {
  try {
