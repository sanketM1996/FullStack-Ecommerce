...
import { toast } from "react-toastify"; // ✅ FIX: toast import
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    // ✅ Load saved checkout address from localStorage
    const savedAddress = JSON.parse(localStorage.getItem("CHECKOUT_ADDRESS"));
    if (savedAddress) {
      dispatch({ type: "SELECT_CHECKOUT_ADDRESS", payload: savedAddress });
    }
  }, [dispatch]);

  // ✅ Auto-select first address if available and none is selected
  useEffect(() => {
    if (address?.length > 0 && !selectedUserCheckoutAddress) {
      dispatch({ type: "SELECT_CHECKOUT_ADDRESS", payload: address[0] });
      localStorage.setItem("CHECKOUT_ADDRESS", JSON.stringify(address[0]));
    }
    ...
    <div className="py-14 min-h-[calc(100vh-100px)]">
      ...
      <Stepper
        ...
        sx={{
          ...
          "& .MuiStepIcon-root.Mui-active": { color: "#059669" },
